...
/********************************************************************************************************
 * This class implements the IPSA-Adapter for maintenance tools like e.g. ISPRO_NG
 * 
...
public class IPSADevice extends BaseDevice {
	...
	public String getAssetTypeFromResources()
	{
		...
		try {
			URL resource = IPSADevice.class.getClassLoader().getResource("MyAssetType.json");
	 ...
	 * IPSARunner
	 ********************************************************************************************************/
	private class IPSARunner implements Runnable {
	...
	public void start() {
		if (!isActive()) {
			...
			// TODO: start ISPRO_NG and other IPSA communication

			//System.out.println(String.format("Starting IPSA %s with speed (%s)", id, Double.toString(speed)));
			active = true;

			new Thread(new IPSARunner()).start();
		}
	}
	...
	public void stop() {
		if (isActive()) {
			...
			// TODO: end ISPRO_NG and other IPSA communication

			//System.out.println(String.format("Stopping IPSA %s, current speed setting (%s)", id, Double.toString(speed)));
			active = false;
		}
	}
	 ...
	 * Listener
	 ********************************************************************************************************/
	public IPSAListener getListener() {
		return listener;
	}
	public void setListener(IPSAListener listener) {
